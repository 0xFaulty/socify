...
public class TransformerVk {
    ...
    public static List<Channel> groupsToChannels(List<GroupFull> groups, Source source) {
        List<Channel> channels = new ArrayList<>();
        for (GroupFull group : groups) {
            channels.add(new Channel(
                    group.getId(), group.getName(), GROUP_PREFIX + group.getId(), group.getPhoto200(), source)
            );
        }

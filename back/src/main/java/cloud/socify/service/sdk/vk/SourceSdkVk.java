...
public class SourceSdkVk implements SourceSdk {
    ...
    public List<Publication> getPublications(Channel channel, int count, int offset) {
        try {
            UserActor actor = new UserActor(Integer.parseInt(userAuthData.getUserId()), userAuthData.getAccessToken());
    ...
    public List<Channel> getChannels(List<String> groupNames, UserAuthData userAuthData, Source source) {
        try {
            UserActor actor = new UserActor(Integer.parseInt(userAuthData.getUserId()), userAuthData.getAccessToken());
            List<GroupFull> groups = vk.groups().getById(actor)
                    ...
                    .execute();
            return TransformerVk.groupsToChannels(groups, source);
        } catch (Exception e) {
            throw new RuntimeException();
        }
    }
